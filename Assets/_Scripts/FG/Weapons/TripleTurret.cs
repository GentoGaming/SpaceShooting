...
{
    public class TripleTurret : MonoBehaviour, IWeapons
    {
        ...
        private void Start()
        {
            ...
            _audioSource = WeaponsManager.Instance.AudioSource;
        ...
        public void LaunchProjectile()
        {
            for (int i = 0; i < 3; i++)
            {
                _bullet = _poolManager.SpawnObjFromPool(bulletInfo.poolTag,
                    transform.TransformPoint(Vector3.zero) + (Vector3) bulletInfo.position[i], bulletInfo.rotation);
            }
        }
