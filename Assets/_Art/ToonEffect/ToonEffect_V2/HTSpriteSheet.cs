...
public class HTSpriteSheet : MonoBehaviour {
	...
	void Awake(){
		...
		// Add light
		if (addLightEffect){
			gameObject.AddComponent(Type.GetType("Light"));
			gameObject.transform.GetComponent<Light>().color = lightColor;
			gameObject.transform.GetComponent<Light>().range = lightRange;
		}
